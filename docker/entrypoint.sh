...
if [ -z "${BLOCKED}" ] && [ ! -f "${CONF}" ]; then
    ...
    RPC_PORT="${RPC_PORT:-32647}"
    # P2P listen port. Written explicitly (rather than left to the daemon's
    # chain default) so it always matches the Docker port published for it
    # (see docker-compose.yml) — the one thing that makes this node
    # reachable for INBOUND peers, which matters for well-connected/
    # validator use (e.g. FlowMesh). Change B3_P2P_PORT (and republish the
    # matching port) if your router only forwards a specific range.
    B3_P2P_PORT="${B3_P2P_PORT:-5647}"
